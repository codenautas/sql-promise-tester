...
sqlPromiseTester = function(motor, opts){
    ...
    describe('sql-promise-tester '+motor.motorName, function(){
        ...
        describe('connectected tests', function(done){
            ...
            it('must fail if multiple data when call fetchUniqueRow',function(done){
                var cursor=conn.query(
                    "SELECT * FROM example1"
                    ...
                    expect(err.message).to.match(/ERROR sql-promise.fetchUniqueRow/);
                    done();
                }).catch(done);
            });
            it('must fail if multiple data when call fetchUniqueRow with no rows',function(done){
                var cursor=conn.query(
            ...
            it('must select data when call fetchUniqueRow',function(done){
                var cursor=conn.query(
                    "SELECT datum FROM example1 WHERE id_num=3"
                ).fetchUniqueRow().then(function(result){
                    expect(result).to.eql({datum: 'three'});
                    done();
                },function(err){
                    done(Error(err));
                }).catch(done);
            });
            it('must fail if multiple data when call fetchUniqueValue',function(done){
                var cursor=conn.query(
                    "SELECT * FROM example1"
                    ...
                    expect(err.notUnique).to.ok();
                    expect(err.message).to.match(/ERROR sql-promise.fetchUniqueValue/);
                    done();
                }).catch(done);
            });
            it('must fail if multiple data when call fetchUniqueValue with no rows',function(done){
                var cursor=conn.query(
                    "SELECT * FROM example1 where 0<0"
                ).fetchUniqueValue().then(function(result){
                    done(Error('must fail, it haves rows'));
            ...
            it('must select data when call fetchUniqueValue',function(done){
                var cursor=conn.query(
                    "SELECT datum FROM example1 WHERE id_num=6"
                ).fetchUniqueValue().then(function(result){
                    expect(result).to.eql('six');
                    done();
                },function(err){
                    done(Error(err));
                }).catch(done);
