...
sqlPromiseTester = function(motor, opts){
    ...
    describe('sql-promise-tester '+motor.motorName, function(){
        ...
        describe('connectected tests', function(done){
            ...
            function testExecute(txt, sqlSentence, expectedValue, done) {
            ...
            testExecute('create table must return rowCount:0', "CREATE TABLE example2(id_num integer primary key, datum text)", {rowCount:0}, done);
            testExecute('insert 1 must return rowCount:1', "INSERT INTO example2 VALUES('1', 'the one')", {rowCount:1}, done);
            testExecute('insert 2 must return rowCount:1', "INSERT INTO example2 VALUES('2', 'el dos')", {rowCount:1}, done);
            testExecute('insert 4 must return rowCount:1', "INSERT INTO example2 VALUES('4', 'die vierte')", {rowCount:1}, done);
            testExecute('delete 1 must return rowCount:1', "DELETE FROM example2 WHERE id_num = '1'", {rowCount:1}, done);
            testExecute('delete 2 and 4 must return rowCount:2', "DELETE FROM example2 WHERE id_num <> 1", {rowCount:2}, done);
        });
    });
};
