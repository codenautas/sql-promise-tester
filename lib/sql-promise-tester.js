...
sqlPromiseTester = function(motor, opts){
    ...
    describe('sql-promise-tester '+motor.motorName, function(){
        ...
        describe('connectected tests', function(done){
            ...
            it('must fail if multiple data when call to fetchUniqueRow',function(done){
                var cursor=conn.query(
                    "SELECT * FROM example1"
            ...
            it('must fail if multiple data when call to fetchUniqueRow with no rows',function(done){
                var cursor=conn.query(
            ...
            it.skip('must select requested data when call to fetchUniqueRow',function(done){
                var cursor=conn.query(
                    "SELECT datum FROM example1 WHERE id_num=3"
                ).fetchUniqueRow().then(function(result){
                    console.log("res", result)
                    // expect(result.notUnique).to.ok();
                    // expect(err.message).to.match(/ERROR sql-promise.fetchUniqueRow/);
                    done();
                },function(err){
                    done(Error(err));
